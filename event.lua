...
function Garrison:FullUpdateTalents()
    local tmpTalents = {}
    local talentTreeID = C_Garrison.GetTalentTreeIDsByClassID(LE_GARRISON_TYPE_7_0, select(3, UnitClass("player")))
    local talentArr = C_Garrison.GetTalentTreeInfoForID(talentTreeID[1])
    ...
    local tmpWarTalents = {} 
    local warTalentTree
    if charInfo.playerFaction == "Alliance" then 
            warTalentTree = 153
        else
            warTalentTree = 152
    end
    local _, _, warTalentArr = C_Garrison.GetTalentTreeInfoForID(warTalentTree)

    if warTalentArr ~= nil then
        local wartalents = warTalentArr;

        for i = 1, #wartalents do
            local talent = wartalents[i]
            local tmpTalent = Garrison:UpdateTalent(talent)

            if tmpTalent ~= nil then
                local talentId = tmpTalent.id
                debugPrint(("UpdateWarTalent: %s / %s"):format(talentId or "-1", talent.name))
                tmpWarTalents[talentId] = tmpTalent
            end
        end

        globalDb.data[charInfo.realmName][charInfo.playerName].warTalents = tmpWarTalents
    end
end
